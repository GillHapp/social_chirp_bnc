...
contract ChirpingContract {
    ...
    function addChirping(
        uint _numOfCharacters,
        string memory _chirpingText,
        string memory _chirpingImage
    ) public {
        require(checkUser(msg.sender) == true, "Add user");
        User storage creator = usernameToUser[msg.sender];
        require(
            block.timestamp - creator.prevTimestamp >= timestamp(creator.level),
            "You have to wait"
        );
        ...
        require(
            _numOfCharacters > 0 && _numOfCharacters < maxCharNum,
    ...
    function addUser(address user, string memory displayPicture) public {
        require(
            usernameToUser[user].username == address(0),
            "User already exists"
        );
        string memory _name = "";
        usernameToUser[user] = User(
            userId,
            user,
            0,
            0,
    ...
    function getAllChirpings() public view returns (ChirpingReturn[] memory) {
        ...
        ChirpingReturn[] memory allChirpings = new ChirpingReturn[](
            trueChirpings
        );
        uint currIndex = 0;
        for (uint i = 0; i < chirpingId; i++) {
            ...
            if (currChirping.hide == false) {
                ...
                allChirpings[currIndex].chirpingText = currChirping
                    .chirpingText;
                ...
                allChirpings[currIndex].chirpingImage = currChirping
                    .chirpingImage;
                allChirpings[currIndex].creatorLevel = currChirping
                    .creatorLevel;
                currIndex++;
            }
        }
    ...
    function getMyChirpings(
        address user
    ) public view returns (ChirpingReturn[] memory) {
        uint trueChirpings = 0;
        for (uint i = 0; i < chirpingId; i++) {
            Chirping storage currChirping = chirpingMap[i];
        ...
        ChirpingReturn[] memory myChirpings = new ChirpingReturn[](
            trueChirpings
        );
        uint currIndex = 0;
        for (uint i = 0; i < chirpingId; i++) {
            ...
            if (currChirping.creator == user && currChirping.hide == false) {
                ...
                myChirpings[currIndex].chirpingImage = currChirping
                    .chirpingImage;
    ...
    function getCagedChirpings(
        ...
        ChirpingReturn[] memory myCagedChirpings = new ChirpingReturn[](
            hiddenChirpings
        );
        uint currIndex = 0;
        for (uint i = 0; i < chirpingId; i++) {
            ...
            if (currChirping.creator == user && currChirping.hide == true) {
                myCagedChirpings[currIndex].chirpingId = currChirping
                    .chirpingId;
                myCagedChirpings[currIndex].creator = currChirping.creator;
                myCagedChirpings[currIndex].chirpingText = currChirping
                    .chirpingText;
                ...
                myCagedChirpings[currIndex].chirpingImage = currChirping
                    .chirpingImage;
                myCagedChirpings[currIndex].creatorLevel = currChirping
                    .creatorLevel;
                currIndex++;
            }
        }
    ...
    function givingWings(uint _chirpingId) public {
        ...
        require(
            currChirping.creator != msg.sender,
            "You cannot give wings to yourself"
        );
        require(
            currChirping.wingsGiver[msg.sender] == false,
            "You've already given wings"
        );
        require(
            currChirping.cagesGiver[msg.sender] == false,
            "You've already given cage"
        );
    ...
    function givingCage(uint _chirpingId) public {
        ...
        require(
            currChirping.creator != msg.sender,
            "You cannot give cage to yourself"
        );
        require(
            currChirping.cagesGiver[msg.sender] == false,
            "You've already given cage"
        ...
        if (currChirping.wings < 10 && currChirping.cages < 10) {
            ...
            currChirping.wings >= 10 && currChirping.cages > currChirping.wings
        ) {
            currChirping.cages++;
            chirpingMap[_chirpingId].hide = true;
        } else {
    ...
    function promoteLevel() public {
        ...
        if (
            (user.totalChirpings >= 10 && user.totalChirpings < 20) &&
            (user.totalWings >= 20) &&
            user.level == 0
        ) {
            user.level = 1;
        } else if (
            (user.totalChirpings >= 20 && user.totalChirpings < 30) &&
            (user.totalWings >= 40) &&
            user.level < 2
        ) {
            user.level = 2;
        } else if (
            (user.totalChirpings >= 30 && user.totalChirpings < 50) &&
            (user.totalWings >= 60) &&
            user.level < 3
        ) {
            user.level = 3;
        } else if (
            (user.totalChirpings >= 50 && user.totalChirpings < 70) &&
            (user.totalWings >= 80) &&
            user.level < 4
        ) {
            user.level = 4;
        } else if (
            (user.totalChirpings >= 70 && user.totalChirpings < 100) &&
            (user.totalWings >= 100) &&
            user.level < 5
        ) {
            user.level = 5;
